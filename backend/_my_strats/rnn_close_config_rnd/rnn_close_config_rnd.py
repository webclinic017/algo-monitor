    ...
    for tkr_i in range(ticker_iterations): # rode n vezes por ticker
        ...
        for c in cfgs: # para cada configuração (fará média com todas cfgs - ensemble)
            ...
            for cfg_i in range(config_iterations): # faça n testes para essa configuração
                ...
                df_init.insert(0, f'Close {config["pred_offset"]}', df_init['Close'].shift(-config['pred_offset']))
        ...
        if (len(preds) > 0):
            ...
            #         'avg_mae': avg_mae,
            #         'avg_mape': avg_mape,
            #         'avg_variation': [(df[f'Close {config["pred_offset"]}'] - df['Close']).apply(lambda x: abs(x)).mean()],
            #     }
            # }
            
            # h.mkdir_conditional('results')
            # with open(f'results/{tkr}_pred_close_{pred_day}_{round(time.time())}.json', 'w') as file: file.write(json.dumps(report))
